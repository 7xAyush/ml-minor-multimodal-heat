...
def create_splits(labels_df: pd.DataFrame, config: Dict[str, Any]) -> Tuple[int, int, int]:
    ...
    strategy = splits_cfg.get("strategy", "random")
    group_key = splits_cfg.get("group_key")
    ...
    if strategy == "random":
    ...
    elif strategy in {"group_tile", "group"}:
        if group_key is None:
            raise ValueError(
                "splits.strategy is set to 'group_tile' but splits.group_key is not provided."
            )
        if group_key not in labels_df.columns:
            raise ValueError(
                f"Group key '{group_key}' not found in labels dataframe columns."
            )

        groups = labels_df[group_key]
        unique_groups = groups.unique()
        n_groups = len(unique_groups)
        if n_groups < 3:
            raise ValueError(
                f"Grouped splitting requires at least 3 distinct groups in '{group_key}', "
                f"but found only {n_groups}. Add more tiles or use random splitting."
            )

        logger.info(
            "Using grouped split on '%s' with %d groups.", group_key, n_groups
        )

        # First split groups into train vs temp (val+test)
        gss1 = GroupShuffleSplit(
            n_splits=1,
            train_size=train_ratio,
            random_state=random_state,
        )
        train_idx, temp_idx = next(gss1.split(labels_df, groups=groups))
        train_df = labels_df.iloc[train_idx]
        temp_df = labels_df.iloc[temp_idx]

        temp_groups = temp_df[group_key]
        temp_unique_groups = temp_groups.unique()
        if len(temp_unique_groups) < 2:
            raise ValueError(
                "Not enough groups left for val/test split in grouped strategy. "
                "Increase the dataset size or adjust train/val/test ratios."
            )

        # Now split temp into val and test groups
        relative_test_size = test_ratio / (val_ratio + test_ratio)
        ...
        logger.info(
            "Grouped split sizes (rows): train=%d, val=%d, test=%d",
            len(train_df),
            len(val_df),
            len(test_df),
        )
        ...
        raise ValueError(f"Unknown splits.strategy: {strategy!r}")

    if len(test_df) == 0:
        raise ValueError(
            "Test split is empty after splitting. For strict evaluation with a held-out "
            "test set, please increase the dataset size or adjust train/val/test ratios "
            "so that the test split contains samples."
        )
